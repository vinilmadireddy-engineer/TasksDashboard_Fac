...
def load_excel(file_path, modified_time):
    ...
    loaded_df["Completion Status"] = pd.to_numeric(loaded_df["Completion Status"],errors="coerce").fillna(0).astype(int)
...
if not open_df.empty:
    ...
    edited_data = st.data_editor(
        ...
        column_config={
            ...
            "Completion Status": st.column_config.ProgressColumn(
                "Completion Status",
                min_value=0,
                max_value=100,
                format="%.0f%%",
        ...
        if blank_tasks.any():
            ...
            try:
                ...
                for position in range(total_rows):
                    ...
                    updated_df.loc[original_row_id, "Completion Status"] = int(row["Completion Status"])
                    row_progress = 15 + int(
